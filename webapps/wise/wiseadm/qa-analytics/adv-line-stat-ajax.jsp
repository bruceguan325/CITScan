...
%><%
	...
	try {
		...
		if ("date".equals(xAxisDataField)) {
			...
			List<String> xAxisFacetValues = QAUtil.getAllPossibleFacetTerms(t.getStatisticsSolrServer(), xAxisFacetName, addtionalFilterForGetXAxis);
		...
		multiCoreQ
				...
				.setParam("f.QuestionType_s.facet.limit", "" + yAxisDataNum)
				.setParam("f.AnswerType_s.facet.limit", "" + yAxisDataNum)
				.setParam("f.QuestionKeyword_ms.facet.limit", "" + yAxisDataNum)
				.setParam("f.AnswerCategory_s.facet.limit", "" + yAxisDataNum);
			...
			for (int i = 0; i < seriesArray.length(); i++) {
				...
				if (obj.has("name") && obj.has("data")) {
					...
					for (int j = 0; j < arr.length(); j++) {
						...
						dataArr.put(0, j);
						dataArr.put(1, i);
						dataArr.put(2, arr.get(j));
						seriesArray1.put(dataArr);
					}
				}
			}
