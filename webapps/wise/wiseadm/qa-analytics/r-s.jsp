...
HashMap<String, String> translateMap = new HashMap<String, String>();
{
	...
	translateMap.put("YAHOO_AUCTION", "Y!拍賣");
	translateMap.put("RUTEN_AUCTION", "露天拍賣");
	translateMap.put("blog", "網誌");
	translateMap.put("poll", "投票");
	translateMap.put("event", "活動");
	translateMap.put("link", "連結");
	translateMap.put("video", "視頻");
	translateMap.put("pic", "照片");
	translateMap.put("album", "相冊");
	// translateMap.put("", "");
}
         	...
         	Sort by
         		...
         		for (int sss=0; sss < sortables.length; sss++) {
                 	...
                 	if(columnName !=null && columnName.getColumnName() != null)
                 		...
                 		colName = null;
         		%>
         		&nbsp;&nbsp;<A HREF="<%= makeUrl(request, forwardJSP, basicMakeUrlParams + "&" + sortByQuery.setSortField(sortables[sss], SolrQuery.ORDER.desc).toString()) %>"><%= translate(colName) %> <img src="img/icon_desc.gif"></A>
            ...
            <table class="table table-striped">
            	<tr valign="top">
                	<td class="op-title col-md-4">
						<a onmousedown="return clk(this.href, this.title)" target="_new" HREF="<%= detailUrl %>" title="<%= StringEscapeUtils.escapeHtml(title) + "," + MessageUtil.getMessage(locale, "open.new.window") %>" <%= target == null ? "" : ("target=\"" + target + "\"") %>>
