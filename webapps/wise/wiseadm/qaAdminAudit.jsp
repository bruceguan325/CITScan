...
%>
<%!
static final String THIS = "qaAdminAudit.jsp";
static final String EDITOR = "qaDataEditor.jsp";
static final String DATATYPE = QAUtil.DATATYPE_QA_AUDIT;
static final String PROCESS = "auditProcess";

static Collection<String> POSSIBLE_FIELDNAMES = Arrays.asList(new String[] {"id", "kid_l", "Name_t", "ANSWER_PLUGIN_ID_s", "AltCount_i", "QUESTION_s", "Status_s", "ANSWER_s", "Title_t", "Date_dt", "TopCategoryName_s", QA.FN_BANNED, QA.FN_IS_NOT_FOR_MLT, QA.FN_IS_NOT_FOR_SEARCH, QA.FN_NOT_APPEND_MLT, "Category_ms", "Name", "LogTime", "updatedAdminUserId_i", "updated_dt", "ANSWER_LINK_s", "ANSWER_RICHTEXT_s"});
...
<div class="container">
	<%
		...
		if ("deleteSelected".equals(mode) && (AdminGroupFacade.getInstance().getFromSession(session).getSystemAdminCURD() & AdminGroup.O5) > 0) {
	...
	sqry.addFilterQuery("!updatedAdminUserId_i:" + admUser.getId());
	if (showExpired)
	...
	<div class="col-md-9">
		...
		for (Iterator<SolrDocument> docItr = docList.iterator(); docItr.hasNext(); docOffset++) {
			...
			<TR>
                ...
                <td style="word-break: break-all;">
                	<font color="green">審核中</font></br>說明:<%= valStr %>
                </td>
				<% } %>
	...
	<div class="col-md-12">
		...
		<% if (!showExpired && (AdminGroupFacade.getInstance().getFromSession(session).getSystemAdminCURD() & AdminGroup.O5) > 0) { %>
		&nbsp;&nbsp;&nbsp;
		<div class="btn-group">
