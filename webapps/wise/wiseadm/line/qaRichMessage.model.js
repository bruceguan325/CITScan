...
var FlexMessageModel = NestedModel.extend({
	name: "FlexMessageModel",
	defaults: { type: "flex", contents: {} },
	model: {},
	validate: function(attrs, options) {
		if (!attrs.altText || attrs.altText == '') {
			return 'AltText is required';
...
var RichMessageModel = NestedModel.extend({
	...
	urlRoot: '/wise/wiseadm/line/richMessages',  // context path 是個坑？
	defaults: {
		id: null,
		mkey: null,
		tenantId: null,
	...
	parse: function(data) {
		...
		else if (data.msgType == 'flex') {
			data.msgTemplate = new FlexMessageModel(data.msgTemplate, {parse: true});
		}
		return data;
	}
});
...
var RichMessageCollection = Backbone.Collection.extend({
	url: '/wise/wiseadm/line/richMessages',
