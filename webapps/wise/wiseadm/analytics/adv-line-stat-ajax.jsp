...
%><%
	...
	try {
		...
		if ("date".equals(xAxisDataField)) {
			...
			for (; rgStart.before(endDate); rgStart.add(gapType, gapAmount), rgEnd.add(gapType, gapAmount)) {
				if (facetQuery.size() > 100) {
					result.put("warning", mr.getMessage(locale, "search.view.top100"));
					break;
				}
			...
			String xAxisFacetName = au.findFieldNameByParam(xAxisDataField);
		...
		for (String fn: au.getFacets()) {
			multiCoreQ.setParam("f." + fn + ".facet.limit", "" + yAxisDataNum);
		}
