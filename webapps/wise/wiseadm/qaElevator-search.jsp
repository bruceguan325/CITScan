...
<%@ page import="com.intumit.solr.qparser.*"%>
...
%>
<%!
	private static final String FACET_RESULTS_JSP = "qaElevator-search-r.jsp";
...
<%
	...
	String qt = StringUtils.defaultString(request.getParameter("qt"), "/browse");
	...
	if (dsList.size() == 0) {
		out.println(MessageUtil.getMessage((Locale)request.getSession().getAttribute("org.apache.struts.action.LOCALE"), "please.set.the.data.group"));
		return;
	}
	if (targetDsId == 0) {
	...
	for (DataSet ds : dsList) {
		...
		multiCoreQ.setQuery(qqq)
				...
				.setParam("qf", QAUtil.QUERY_BOOST)
				.setParam("mm", "0")
		...
		if (ds.getId() != targetDsId) {
			...
			//模糊搜尋功能開關，如果筆數為0，改為模糊搜尋
			if (ds.getFuzzySearch() != null && ds.getFuzzySearch() && ds.getFuzzyField() != null && ds.getFuzzyFieldWeight() != null) { 
