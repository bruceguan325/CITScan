...
.chatbot-area .wc-message-from-bot .character-icon {
    ...
    height: 70px;
    float: left;
    background: url(./images/icn_intumit_service01.png) 0 0 no-repeat;
}
...
<script>
  <% if(redirectToChatbot) { %>
      window.location.href = "<%=request.getContextPath()%>/helpdesk/chatbot.jsp?qaId=<%=qaId%>";
  <%}%>
</script>
...
<div id="wrap">
  <header id="header">
    <div id="header-logo" style="padding-top: 40px;"><a itemprop="url" href="http://www.intumit.com/japanese/SmartRobot.html" target="_blank"></a></div>
    <div id="header-utility" class="clearfix">
      <div id="header-menu">
        <p id="header-logo" style="width:550px;"></p>
      </div>
    </div>
    <div id="header-title">
      <p>デモサイト</p>
...
<div style="display: none;" id="faq-div">
<div>
<% for(String faq : faqs) { %>
  ...
  if (message.length !== 0 || welcome) {
    ...
    $.ajax({
    	...
    	success: function(resp) {
		  ...
		  if (welcome) {
			  $('#faq-div').find('div').clone().appendTo(reply.find('.format-html'));
		  }
