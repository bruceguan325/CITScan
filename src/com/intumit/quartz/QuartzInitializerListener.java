...
import com.intumit.solr.util.SqlScriptRunner;
...
public class QuartzInitializerListener implements ServletContextListener {
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        if (!doInit(sce)) {
        	log.warn("Trying to populate quartz db schema...");
        	ResourceBundle res = ResourceBundle.getBundle("quartz");
        	String dsName = StringUtils.trimToEmpty(res.getString("org.quartz.jobStore.dataSource"));
        	String jdbcDriver = StringUtils.trimToEmpty(res.getString("org.quartz.dataSource." + dsName + ".driver"));
        	String jdbcUrl = StringUtils.trimToEmpty(res.getString("org.quartz.dataSource." + dsName + ".URL"));
        	String jdbcUser = StringUtils.trimToEmpty(res.getString("org.quartz.dataSource." + dsName + ".user"));
        	String jdbcPass = StringUtils.trimToEmpty(res.getString("org.quartz.dataSource." + dsName + ".password"));
        	
        	String scriptFilename = null;
        	
        	if (jdbcDriver.indexOf("h2") != -1) {
        		scriptFilename = "tables_h2.sql";
        	}
        	else if (jdbcDriver.indexOf("mysql") != -1) {
        		scriptFilename = "tables_mysql_innodb.sql";
        	}
        	...
        	try {
        		...
        		Reader r = new InputStreamReader(QuartzInitializerListener.class.getResourceAsStream(scriptFilename));
        		
        		SqlScriptRunner runner = new SqlScriptRunner(conn, true, true);
        		runner.runScript(r);
        	}
        	catch (Exception e) {
        		log.error("Cannot populate quartz db script", e);
        	}
        	
        	log.warn("Now initialize the quartz agian.");
        	if (!doInit(sce)) {
        		log.error("Quartz database initialize failed.");
        	}
        }
    }
    
    boolean doInit(ServletContextEvent sce) {
