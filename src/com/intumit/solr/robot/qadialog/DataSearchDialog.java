 ...
 * @author herb
 */
public class DataSearchDialog extends QADialog {
			...
			if (doSearchAndListResult || status == InnerStatus.CHECK_INTENT_AND_ENTITY) {
				...
				try {
					...
					while (fItr.hasNext()) {
						...
						else if (f.getDefaultShow() != null && f.getDefaultShow().booleanValue()) {
							fItr.remove();
						}
					}
					
					List<Field> defaultShowFields = new ArrayList<Field>();
					for (Field f: fields) {
						if (f.getDefaultShow() != null && f.getDefaultShow().booleanValue()) {
							defaultShowFields.add(f);
						}
					}
					...
					if (currentFields.size() == 0) {
						...
						currentFields.addAll(0, defaultShowFields);
					}
					...
					if (hasNewMatch || doSearchAndListResult) {
						...
						if (hasNewMatch || appendDocList) {
							...
							if (StringUtils.isEmpty(showSearchResultScript) && qaPatternMkey != null) {
								...
								int c = 1;
								for (SolrDocument doc: currentDocs) {
									if (currentDocs.size() > 1) buf.append("" + (c++) + ". ");
									boolean first = true;
									...
									for (Field f: currentFields) {
										...
										Object value = doc.getFirstValue(f.getTargetIndexFieldName());
										Map<String, Object> bundle = new HashMap<String, Object>();
										bundle.put("currentDoc", doc);
										bundle.put("field", f);
								...
								ctx.setAnswerText(buf.toString());
								ctx.setHasDirectAnswer(true, ANSWER_TYPE.DIALOG);
								ctx.setResponseAttribute("DIALOG", getName());
								status = InnerStatus.CHECK_INTENT_AND_ENTITY;
							}
		...
		try {
			...
			if (StringUtils.isNotEmpty(dialogJson)) {
				...
				for (int i=0; i < dialogs.length(); i++) {
					...
					QADialog ffd = new DataSearchDialog();
					ffd.init(cfg);
	...
	boolean findAndGatherFilters(QAContext ctx, List<Filter> allFilters, List<Filter> targetFilters) {
		...
		if (entityDbs != null) for (QAEntity eid: entityDbs) {
