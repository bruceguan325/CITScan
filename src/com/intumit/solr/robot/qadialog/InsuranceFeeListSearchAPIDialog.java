 ...
 * @author herb
 */
public class InsuranceFeeListSearchAPIDialog extends QADialog {
	...
	public InsuranceFeeListSearchAPIDialog() {
			...
			if (callStack.size() > 0) {
				...
				if (!stackData.to.isActive()) {
					...
					if (stackData.to.getName().equals("InsuranceFeeDetail")) {
						...
						Field idField = stackData.to.getField("id");
						if (idField.getCurrentValue() != null) {
							int offset = (int)(idField.getCurrentValue().charAt(0)) - 65;
							if (offset >= 0 && offset < userNameList.length) {
								ctx.setCtxAttr("userName", userNameList[offset]);
							}
						}
	...
	private void getDetails(SolrDocumentList docs, String userId, List<String> pnos) {
		// https://202.154.204.113/cxlpoc/sb/Invest/QueryByNum?client_id=0fad548e-e92a-4d9d-a76f-07d9cb543bce&id=API0010&POLICY_NO_QRY=7228221028&QRY_FRM=FROMCSRP&QRY_ID=A123456789

		if (!userId.matches("[a-zA-Z0-9]+"))
			return;
		
		File docsCache = new File(WiSeEnv.getHomePath() + "/cache/cxl/insuranceFeeDetail.docs." + userId + "");
		File docsCacheJson = new File(WiSeEnv.getHomePath() + "/cache/cxl/insuranceFeeDetail.docs." + userId + ".json");
		
		if (docsCacheJson.exists()) {
			FileReader fr = null;
			JSONDeserializer jd = new JSONDeserializer();
			
			try {
				fr = new FileReader(docsCacheJson);
				List<Map<String, Object>> cachedDocs = (List<Map<String, Object>>)jd.deserialize(fr);
				
				if (cachedDocs != null) {
					for (Map<String, Object> cachedDoc: cachedDocs) {
		...
		if (docs.size() != pnos.size()) {
			for (String pno: pnos) {
				String apiUrl = "https://202.154.204.113/cxlpoc/sb/Invest/QueryByNum?client_id=0fad548e-e92a-4d9d-a76f-07d9cb543bce&id=API0010&QRY_FRM=FROMCSRP&QRY_ID=A123456789&POLICY_NO_QRY=" + pno;
				try {
					String str = WiSeUtils.getDataFromUrl(apiUrl);
					JSONObject jObj = new JSONObject(str);
					String[] names = JSONObject.getNames(jObj);
					SolrDocument doc = new SolrDocument();
					
					for (String name: names) {
						String fn = fieldNameMapping.containsKey(name) ? fieldNameMapping.get(name) : name;
						
						if (fn.endsWith("_dt") || fn.endsWith("_DATE")) {
							SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
							doc.setField(fn, sdf.parse(jObj.getString(name)));
						}
						else if (fn.equals("ProductId_s")) {
							String productId = jObj.getString(name);
							
							if (productId.matches("(?ism)^[A-Z]+[0-9]")) {
								productId = productId.substring(0, productId.length() - 1);
							}
							if (prdNameMap.containsKey(productId)) {
								doc.setField("ProductName_s", prdNameMap.get(productId));
							}
							else {
								doc.setField("ProductName_s", productId);
							}
						}
						else if (fn.equals("effectiveness_s")) {
							doc.setField(fn, effectivenessMapping.get(new Integer(jObj.getString(name)).toString()));
						}
						else if (fn.equals("period_s")) {
							doc.setField(fn, periodMapping.get(jObj.getString(name)));
						}
						else {
							doc.setField(fn, jObj.get(name));
						}
			...
			if (docs.size() > 0) {
				...
				FileWriter fw = null;
				try {
					fw = new FileWriter(docsCacheJson);
					js.deepSerialize(docs, fw);
					fw.close();
				} catch (IOException e) {
					e.printStackTrace();
				} finally {
		...
		for (SolrDocument doc: docs) {
			if (doc.containsKey("PREM_RCPT")) {
				double rcpt = Double.parseDouble((String)doc.getFirstValue("PREM_RCPT"));
				if (Math.ceil(rcpt) == rcpt) {
					doc.setField("PREM_RCPT", "" + (int)rcpt);
				}
			}

			// 客製欄位
			try {
				String sPayprd = (String)doc.getFirstValue("PAY_PRD");
				String sPaytimes = (String)doc.getFirstValue("PAY_TIMES");
				String sPayfreq = (String)doc.getFirstValue("period_s");
				
				int iPayprd = Integer.parseInt(sPayprd);
				int iPaytimes = Integer.parseInt(sPaytimes);
				
				Long issueDateLong = (Long)doc.getFirstValue("IssueDate_dt");
				
				if (issueDateLong != null) {
					Date issueDate = new Date(issueDateLong);
					Calendar issueCal = Calendar.getInstance();
					issueCal.setTime(issueDate);
					int issueYear = issueCal.get(Calendar.YEAR);
					
					doc.put("PAY_TO_YEAR", "" + (issueYear + iPayprd));
					
					// 計算還要繳幾次
					if (sPayfreq != null) {
						int timePerYear = -1;
						if (sPayfreq.equals("月繳")) {
							timePerYear = 12;
						}
						else if (sPayfreq.equals("季繳")) {
							timePerYear = 4;
						}
						else if (sPayfreq.equals("半年繳")) {
							timePerYear = 2;
						}
						else if (sPayfreq.equals("年繳")) {
							timePerYear = 1;
						}
						
						if (timePerYear != -1) {
							int totalTimes = iPayprd * timePerYear;
							int remainTimes = totalTimes - iPaytimes;
							
							doc.put("PAY_REMAIN_TIMES", "" + remainTimes);
						}
					}
				}
			} catch (Exception e) {
	...
	private List<String> getPolicyNosByUserId(String userId) {
		//https://202.154.204.113/cxlpoc/sb/Invest/QueryByID?client_id=0fad548e-e92a-4d9d-a76f-07d9cb543bce&id=API0009&CLIENT_ID=Y18664628C&ROLE=A

		List<String> l = new ArrayList<String>();
		if (!userId.matches("[a-zA-Z0-9]+"))
			return null;
		
		File pnos = new File(WiSeEnv.getHomePath() + "/cache/cxl/pnos." + userId + "");
		
		if (pnos.exists()) {
			try {
				String pnosStr = FileUtils.getContentsAsString(pnos, "UTF-8");
				l.addAll( Arrays.asList(StringUtils.split(pnosStr, ",")) );
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		if (l.size() == 0) {
			String str = null;
			try {
				String apiUrl = "https://202.154.204.113/cxlpoc/sb/Invest/QueryByID?client_id=0fad548e-e92a-4d9d-a76f-07d9cb543bce&id=API0009&ROLE=A&CLIENT_ID=" + StringUtils.upperCase(userId);
				str = WiSeUtils.getDataFromUrl(apiUrl);
				JSONArray jArr = new JSONArray(str);
				
				for (int i=0; i < jArr.length(); i++) {
					JSONObject obj = jArr.getJSONObject(i);
					
					if (obj.has("policy_no")) 
						l.add(obj.getString("policy_no"));
				}

				if (l.size() > 0) {
					try {
						IOUtils.write(StringUtils.join(l, ","), new FileOutputStream(pnos), "UTF-8");
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				...
				if (str != null) {
					try {
						JSONObject jErr = new JSONObject(str);
						String errMsg = jErr.getString("errMsg");
	...
	SolrDocumentList searchAndApplyFilters(QAContext ctx) {
		if (allDocsOfThisUser == null) {
			String userId = getField("id").getCurrentValue();
			allDocsOfThisUser = new SolrDocumentList();
			
			// https://202.154.204.113/cxlpoc/sb/Invest/QueryByID?client_id=0fad548e-e92a-4d9d-a76f-07d9cb543bce&id=API0009&CLIENT_ID=Y18664628C&ROLE=A
			List<String> pnos = getPolicyNosByUserId(userId);
			getDetails(allDocsOfThisUser, userId, pnos);
		}
