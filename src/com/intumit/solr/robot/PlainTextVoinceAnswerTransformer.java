...
public class PlainTextVoinceAnswerTransformer implements AnswerTransformer {
	...
	public QAContext transform(QAChannel ch, QAContext ctx) {
		...
		if (ctx.getCurrentQA() != null) {
			...
			MultiChannelAnswer mca = MultiChannelAnswer.get(ctx.getTenant().getId(), (String) currentQA.getFieldValue("id"),
					ctx.getQaChannel(), ctx.getUserType());
		...
		else if (ctx.hasAnswerText() && ANSWER_TYPE.NO_ANSWER == ctx.getAnswerType()) {
			ctx.setResponseAttribute("outputVoice", TemplateUtil.processTwice(ctx, ctx.getAnswerText().toString(), far));
		}
