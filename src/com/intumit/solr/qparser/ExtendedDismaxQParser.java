...
public class ExtendedDismaxQParser extends QParser {
  ...
  public Query parse() throws SyntaxError {
	...
	try {
		String origin = userQuery;
		userQuery = SynAndSegQueryUtils.doSegAndSyn(config.tenantId == null ? WiseSystemConfig.get().getDefaultTenantId() : config.tenantId, req.getCore(), userQuery, config.doSegment, config.doSyn);
		if (DEBUG) System.out.println("After SynAndSeg (" + config.doSyn + "/" + config.doSegment + ") :[" + origin + "] -> [" + userQuery + "]");
	}
	catch (Exception e) {
		e.printStackTrace();
	}
    ...
    if( userQuery == null || userQuery.trim().length() == 0 ) {
      ...
      if (config.doAutoPhrase != null)
    	  up.setAutoGeneratePhraseQueries(config.doAutoPhrase);
      ...
      //if (DEBUG) System.out.println("mainUserQuery:" + mainUserQuery);
      ...
      if (DEBUG) {
    	  System.out.println("parsedUserQuery:" + parsedUserQuery.getClass().getName() +":" +parsedUserQuery);
      }
  ...
  protected ExtendedSolrQueryParser createEdismaxQueryParser(QParser qParser, String field) {
    return new ExtendedSolrQueryParser(qParser, field, config.bigramMode);
  }
  ...
  protected static class Clause {
	...
	public String toString() {
		return "Clause [field=" + field + ", rawField=" + rawField
				+ ", isPhrase=" + isPhrase + ", hasWhitespace=" + hasWhitespace
				+ ", hasSpecialSyntax=" + hasSpecialSyntax + ", syntaxError="
				+ syntaxError + ", must=" + must + ", val=" + val + ", raw="
				+ raw + "]";
	}
  ...
  public static class ExtendedSolrQueryParser extends SolrQueryParser {
    ...
    private float flt;
    private int slop;
	private boolean bigramMode;
    
    public ExtendedSolrQueryParser(QParser parser, String defaultField, boolean bigramMode) {
      super(parser, defaultField);
      // don't trust that our parent class won't ever change it's default
      setDefaultOperator(QueryParser.Operator.OR);
      this.bigramMode = bigramMode;
    }
    ...
    protected Query getWildcardQuery(String field, String val) throws SyntaxError {
      ...
      if (!bigramMode && StringUtils.containsAny(val, new char[] {'?', '*'}) && SynAndSegQueryUtils.isChinese(val.charAt(0))) {
    	  int matches = StringUtils.countMatches(val, "?");
  ...
  public class ExtendedDismaxConfiguration {
    ...
    protected boolean doSyn;
    protected boolean doSegment;
    protected Boolean doAutoPhrase;
    protected boolean bigramMode;
    ...
    public ExtendedDismaxConfiguration(SolrParams localParams,
      ...
      bigramMode = solrParams.getBool(ExtendedDismaxQParserPlugin.BIGRAM_MODE, false);
      tenantId = solrParams.getInt(ExtendedDismaxQParserPlugin.TENANT_ID);
      doSyn = solrParams.getBool(ExtendedDismaxQParserPlugin.SYNONYM, true);
      doSegment = solrParams.getBool(ExtendedDismaxQParserPlugin.SEGMENT, false);
      
      Boolean b = solrParams.getBool(ExtendedDismaxQParserPlugin.NO_AUTO_PHRASE);
      doAutoPhrase = b == null ? null : !b;
