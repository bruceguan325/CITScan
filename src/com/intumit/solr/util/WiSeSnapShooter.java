 ...
 * @since solr 1.4
 */
public class WiSeSnapShooter {
	private static final Logger LOG = LoggerFactory.getLogger(WiSeSnapShooter.class.getName());
	...
	public WiSeSnapShooter(SolrCore core, String location, String snapshotName) {
		solrCore = core;
		if (location == null)
	...
	void createSnapAsync(final IndexCommit indexCommit, final int numberToKeep,
			final WiSeReplicationHandler replicationHandler) {
	...
	public void validateDeleteSnapshot() {
		...
		if (dirFound == false) {
			throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
					"Snapshot cannot be found in directory: " + snapDir);
		}
	}

	protected void deleteSnapAsync(final WiSeReplicationHandler replicationHandler) {
		new Thread() {
			@Override
			public void run() {
	...
	void createSnapshot(final IndexCommit indexCommit, WiSeReplicationHandler replicationHandler) {
		...
		try {
			...
			Directory dir = solrCore.getDirectoryFactory().get(solrCore.getIndexDir(), DirContext.DEFAULT,
			...
			WiSeSnapPuller.delTree(snapShotDir);
	...
	private void deleteOldBackups(int numberToKeep) {
		...
		for (OldBackupDirectory dir : dirs) {
			if (i++ > numberToKeep - 1) {
				WiSeSnapPuller.delTree(dir.dir);
			}
		}
	}

	protected void deleteNamedSnapshot(WiSeReplicationHandler replicationHandler) {
		LOG.info("Deleting snapshot: " + snapshotName);

		NamedList<Object> details = new NamedList<>();
		boolean isSuccess;
		File f = new File(snapDir, "snapshot." + snapshotName);
		isSuccess = WiSeSnapPuller.delTree(f);
