...
public class JsonpFilter implements Filter {
	 ...
	 * a browser
	 */
	public static final Pattern SAFE_PATTERN = Pattern
			.compile("[a-zA-Z_0-9\\.]+");
	...
	public void doFilter(ServletRequest req, ServletResponse resp,
		...
		if (callbackValue == null || callbackValue.isEmpty()) {
			...
			logger.debug("Json padding '{}' requested from {}", callbackValue,
					request);
	 ...
	 */
	static class HttpServletResponseContentWrapper extends
		...
		public void setContentType(String type) {
			enableWrapping = StringUtils.startsWithIgnoreCase(type, "application/json");
		...
		public PrintWriter getWriter() throws IOException {
			...
			if (enableWrapping) {
				if (bufferWriter == null) {
					bufferWriter = new PrintWriter(new OutputStreamWriter(
							buffer, this.getCharacterEncoding()));
				}
				return bufferWriter;
			} else {
		...
		public void setBufferSize(int size) {
			...
			if (enableWrapping) {
				// DO NOTHING BCZ WE USE ByteArrayOutputStream;
			} else {
